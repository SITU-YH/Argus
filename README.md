...
1. **配置**:
    ...
    **相机驱动**: [`hikvision_ros2_driver`](https://github.com/SITU-YH/hikvision_driver_ros2) LAN2
...
修改`config/camera_params.yaml`和`config/camera_info.yaml`中的相关参数

修改`config/fast_lio_mid360.yaml`文件中的雷达-IMU外参矩阵`extrinsic_T`, `extrinsic_R` 触发暂不需要
...
修改`config/camera_params`中的相机参数
```bash
# 调整相机参数并同时在rviz中查看画面
